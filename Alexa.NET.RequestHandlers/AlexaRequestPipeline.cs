...
namespace Alexa.NET.RequestHandlers
{
	public class AlexaRequestPipeline<TSkillRequest> where TSkillRequest:SkillRequest
	{
	    ...
	    private Task<SkillResponse> Process(AlexaRequestInformation<TSkillRequest> information)
        {
            ...
            try
			{
				...
				return new AlexaRequestInterceptor<TSkillRequest>(RequestInterceptors.First,candidate).Intercept(information);
			}
			...
			catch (Exception ex)
			{
				...
				return new AlexaErrorInterceptor<TSkillRequest>(ErrorInterceptors.First,candidate,errorCandidate).Intercept(information, ex);
			}
		}
    }
}
